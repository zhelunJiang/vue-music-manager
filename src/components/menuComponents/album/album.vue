<template>
  <section>
        ...
        <el-col :span="24">
          <el-form :inline="true"  class="demo-form-inline" height="8%">
            <el-form-item label="搜索:">
              <el-input placeholder="输入专辑名称" v-model="filter.name"></el-input>
            </el-form-item>
    ...
    <el-row>
      <el-col :span="24">
        <el-table  :data="page.list" size="mini" style="width: 100%" v-loading="loading" stripe>
          <el-table-column type="selection">
          </el-table-column>
          <el-table-column prop="publish" label="发布时间" sortable>
            <template slot-scope="slot">
              {{$dateFormat(slot.row.publish)}}
            </template>
          </el-table-column>
          ...
          <el-table-column  label="图片">
            <template slot-scope="scope">
              <img :src="$basePath+scope.row.img" style="height: 60px;width: 60px">
            </template>
          </el-table-column>
          <el-table-column prop="sname" label="歌手">
          </el-table-column>
          <el-table-column prop="click" label="点击量次">
          </el-table-column>
          <el-table-column prop="search" label="搜索量次">
          </el-table-column>
    ...
    <el-dialog :visible.sync="from.visible"size="tiny" width="25%">
      <album-dialog ref="window" @saved="handleSave"></album-dialog>
    </el-dialog>
  </section>
</template>
<script>
  import API from '../../../api/music/album.js'
  import AlbumDialog from './AlbumEdit.vue'
  export default {
    components: {
      AlbumDialog
    },
